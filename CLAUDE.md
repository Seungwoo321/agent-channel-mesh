...
- **저장 위치는 신원에서 판다.** 기본 디렉토리 아래 지문 한 칸을 붙이고, 사용자가 `store.dir` 을 바꿔도 그 칸은 붙인다. 상수 경로에 두면 한 기계의 두 참여자가 설정 파일을 갈라도 같은 채널 파일·같은 `delivered` 상태를 공유해 오배달이 난다 — 실측된 고장이다. 파생은 `storeOptionsOf` 한 곳이 소유하고, 훅도 같은 함수를 거친다.
