...
실행 중 `whoami`는 `~`를 편 경로를 보여 주고, 핵심 툴 응답에는 확인 가능한 경우 릴레이 상태,
수신 lease 상태, Claude push 상태가 함께 나온다. 하나의 지문 저장소를 소비하는 릴레이 수신기는
한 번에 하나만 허용한다. 중복 어댑터는 수신 루프에 들어가지 않고 충돌 상태를 보고한다. `both`
전달에서는 Claude push가 빠른 경로이고 inbox/store가 내구성 있는 안전망이다. 따라서 push 실패가
보이고 메시지는 버려지지 않는다.
